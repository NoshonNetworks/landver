...
export const LandList = () => {
		...
		(async () => {
			try {
				if (status == "connected" && address) {
					setLoadingList(true);
					const addresses = await contract.get_lands_by_owner(
						address
					);
					const newLands = [];
	...
	const assignInspector = async () => {
		await contract.connect(account);
		const inspector_address = inspectorToAssign;
		await contract.set_land_inspector(
							...
							return (
                                ...
                                !!land.inspector && (
									...
									{!land.inspector && (
										<div
											onClick={() => {
												setShowAssignInspectorModal(
													true
												);
												setLandToAssignInspector(
													land.id
												);
											}}
				...
				<div className="absolute top-0 left-0 right-0 bottom-0 bg-black bg-opacity-60 flex justify-center items-center">
					<div className="bg-white p-10 rounded-lg">
						...
						<input
							...
							onChange={(e) =>
								setInspectorToAssign(e.target.value)
							}
							className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
							placeholder="0x..."
						/>
					...
					<div className="bg-white p-10 rounded-lg">
						...
						>
							{LandUse.map((landUse, index) => {
								return (
									<option
										key={"unique-land-item-ddfa" + index}
										onClick={() =>
											setCreateLandData({
												...createLandData,
