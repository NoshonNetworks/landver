...
const RegisterLandModal: React.FC<RegisterLandModalProps> = ({ isOpen, onClose, mode, editData }) => {
    ...
    const handleSubmit = async() => {
        try {
            ...
            if (!landUseEnum || !account || !landData.latitude || !landData.longitude || !landData.area) {
    ...
    const handleEdit = async() => {
        try {
            ...
            const landUseEnum = LandUse.find(lu => lu.name === landData.landUse)?.enum;
            if (!landUseEnum || !account || !landData.area) {
                throw new Error("Missing required data");
            }
                        ...
                        {!editData && (
                            <div className="mt-4">
                                ...
                                <input 
                                    ...
                                    onChange={(e) => setLandData({...landData, latitude: Number(e.target.value)})}
                        ...
                        {!editData && (
                            <div className="mt-4">
                                ...
                                <input 
                                    ...
                                    onChange={(e) => setLandData({...landData, longitude: Number(e.target.value)})}
                                    className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" 
                                    placeholder=""
                                />
                            </div>
