...
export function useEvents<ParsedEvent extends ParsedEventsEnum>({
	name,
	triggerRefetch,
	filters,
}: UseEventsParams) {
			...
			try {
				...
				const eventFilters =
					filters?.events.map((event) =>
						num.toHex(hash.starknetKeccak(event))
					) || [];

				const keyFilter = [[...eventFilters]];
				...
				// parsing event
				const abiEvents = starknetEvents.getAbiEvents(
					contracts[name].abi
				);
				...
				for (let i: number = 0; i < eventsRes.events.length; i++) {
					...
					formattedEvents.push({
						...
						parsedEvent: parsedEvent[
							Object.keys(parsedEvent)[0]
						] as unknown as ParsedEvent, // first to unknows because we dont know what event type is on the item
					});
				}
	...
	}, [triggerRefetch, filters?.events, name]);
