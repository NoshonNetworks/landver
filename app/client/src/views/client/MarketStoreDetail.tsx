...
export function MarketStoreDetailClientView() {
	...
	const { contract: LandRegistryContract } = useLandverContract({
		name: "landRegister",
	});
	const { events: listingEvents, isLoading: loadingEvents } =
		useEvents<ListingCreatedEvent>({
			name: "landRegister",
		...
		(async () => {
			try {
				const listingResponse: Listing =
					await LandRegistryContract.get_listing(listingId);
				setListing(listingResponse);
			} catch (error) {
				console.log(error);
			}
		})();
	}, [LandRegistryContract, listingId]);
			...
			<div className="px-6">
				<div className="bg-white rounded-xl">
					<div className="grid grid-cols-1 lg:grid-cols-2 px-3 py-4 gap-4 rounded-xl">
						<div className="rounded-lg min-h-[250px] bg-gray-200"></div>
						...
						<div className="rounded-lg min-h-[250px] bg-white p-4">
							<div className="flex py-1 ">
								...
								<div className="flex justify-end flex-1 items-center">
									<Image
										src={
											"/icons/common/favorite-filled.svg"
										}
										width={22}
										height={22}
										alt=""
							...
							{!loadingEvents &&
								listingEvents.map((item, index) => {
									return (
										<MarketCard
											key={
												"uniquecardkeymarketstoredetailclientdfa" +
												item.eventKey +
												index
											}
											item={item}
