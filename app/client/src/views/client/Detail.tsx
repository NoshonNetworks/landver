...
export default function DetailClientView() {
	...
	const { contract: registerContract } = useLandverContract({
		name: "landRegister",
	});
	...
	useEffect(() => {
		(async () => {
			try {
				setLoading(true);
				const landResponse: Land = await registerContract.get_land(
					landId
				);
	...
	const statusTag:
		...
		| "pending"
		| "rejected"
		| "bought" = Object.entries(land?.status.variant ?? {})
		...
		| "rejected"
		| "bought";
	const landUse = Object.entries(land?.land_use.variant ?? {}).find(
		(entry) => entry[1]
	)?.[0];
					...
					<div className="flex justify-between items-center px-4">
						...
						{loading ? (
							...
							<div className="flex flex-col items-end">
								<div>
									Latitude:{" "}
									{land?.location.latitude.toString()}
								</div>
								<div>
									Longitude:{" "}
									{land?.location.longitude.toString()}
								</div>
							</div>
						)}
					</div>
						...
						{loading ? (
							...
							<div>
								{formatTimestampToDate(
									Number(land?.last_transaction_timestamp)
								)}
							</div>
						)}
					</div>
