...
export function DashboardClientView() {
	...
	const { events: recentEvents, isLoading: eventsLoading } =
		useEvents<ParsedEventsEnum>({
			name: "landRegister",
		...
		<div className="">
			...
			<div className="grid grid-cols-1 md:grid-cols-3 gap-5 px-6">
				<div className="w-full bg-[#6E62E5] rounded-xl">
					<div className="min-h-56 p-4 flex flex-col gap-6 items-start">
						<p className="text-xl md:text-4xl text-white font-medium">
							Discover, transfer and register your lands
						</p>
				...
				<Card
					mainIconColor="green"
					value={`${landsOwned || 0}`}
					landIds={landsAddresses || []}
					unit={""}
					subtitle="Total Owned Land"
					buttonMessage={"View Details"}
					hasIconsMap={true}
				/>
				<Card
					mainIconColor="blue"
					value={balance}
					landIds={landsAddresses || []}
					unit={balanceData?.symbol || ""}
					subtitle="My Balance"
					buttonMessage={"Top Up Balance"}
					hasIconsMap={false}
				/>
			</div>
	...
	return (
		<div className="w-full bg-white rounded-xl">
			<div className="min-h-56 p-4 flex flex-col gap-6 relative">
				...
				<p className="text-xl md:text-xl font-sm text-gray-500">
					{subtitle}
				</p>
				<div className="relative h-[30px]">
					{!!landIds &&
						hasIconsMap &&
						landIdsToShow.map((landId, index) => {
							const imageRandomizer = "adffhhyethtvegq";

							return (
								<div
									key={"land-circles-on-dashboard" + index}
									style={{
										position: "absolute",
										left: 22 * index,
									}}
				...
				<Button variant="whiteWithBorder">{buttonMessage}</Button>
				<div
					className="absolute top-7 right-4 w-[60px] h-[60px] rounded-full flex justify-center items-center"
					style={{
						backgroundColor:
							mainIconColor === "blue" ? "#F2FAFD" : "#F4FDF9",
					}}
				>
					<div className="relative w-[40px] h-[40px] rounded-full">
						{mainIconColor === "blue" && (
							<Image
								src={"/icons/common/stack-green.svg"}
								alt="ether"
								fill
								style={{
									objectFit: "cover",
						...
						{mainIconColor === "green" && (
							<Image
								src={"/icons/common/stack-blue.svg"}
								alt="ether"
								fill
								style={{
									objectFit: "cover",
