...
export default function MyCollectionsClientView() {
	...
	const startDate =
		dateRange && !(dateRange instanceof Date) && dateRange[0]
			? formatDate(dateRange[0])
			: null;
	const endDate =
		dateRange && !(dateRange instanceof Date) && dateRange[1]
			...
			try {
				if (address) {
					...
					for await (const address of addresses) {
						...
						if (landStatus && landStatus[0] === "Bought")
							newLandsCount.bought += 1;
						if (landStatus && landStatus[0] === "Approved")
							newLandsCount.registered += 1;
						if (landStatus && landStatus[0] === "Unapproved")
							newLandsCount.unapproved += 1;
			...
			{!loading && (
				<>
					<div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-5 px-6">
						<SmallNumberCard
							iconColor="blue"
							title={landsCounts.total.toString()}
							subtitle="Total Lands Owned"
						/>
						<SmallNumberCard
							iconColor="yellow"
							title={landsCounts.registered.toString()}
							subtitle="Total Lands Registered"
						/>
						<SmallNumberCard
							iconColor="orange"
							title={landsCounts.bought.toString()}
							subtitle="Total Lands Bought"
						/>
						<SmallNumberCard
							iconColor="purple"
							title={landsCounts.unapproved.toString()}
							subtitle="Total Lands Unapproved"
						/>
					</div>
							...
							<div className="flex justify-between items-start gap-2 flex-wrap">
								...
								<div className="flex gap-3">
									...
									>
										...
										<Image
											src={
												"icons/common/dropdown-grey.svg"
											}
											alt=""
											width={12}
											height={12}
									...
									<div
										onClick={() =>
											setShowDateRangeCalendar(
												!showDateRangeCalendar
											)
										}
										className="cursor-pointer relative flex gap-2 rounded-lg bg-gray-100 px-5 py-2 text-center text-gray-500 flex-shrink-0"
									>
										<Image
											src={
												"icons/common/calendar-gray.svg"
											}
											alt=""
											width={14}
											height={14}
										...
										{showDateRangeCalendar && (
											<div
												className="absolute top-[110%] right-0"
												style={{ zIndex: 10000 }}
												onClick={(e) =>
													e.stopPropagation()
												}
